...
public class SpellingCorrector {
    ...
    public static int editDistance(String a, String b) {
        ...
        Integer[][] twoDeeArray = new Integer[a.length() + 1][b.length() + 1];
        return editDistanceHelper(a, b, twoDeeArray);

    }

    private static int editDistanceHelper(String a, String b, Integer[][] storage) {
        int aLen = a.length();
        int bLen = b.length();
        if (storage[aLen][bLen] != null) {
        ...
        if (a.charAt(0) == b.charAt(0)) {
            storage[aLen][bLen] = editDistanceHelper(a.substring(1), b.substring(1), storage);
            return storage[aLen][bLen];
        }
        else {
            int x = editDistanceHelper(a.substring(1), b, storage);
            int y = editDistanceHelper(a, b.substring(1), storage);
            int z = editDistanceHelper(a.substring(1), b.substring(1), storage);
            storage[aLen][bLen] = Math.min(x, Math.min(y, z)) + 1;
            return storage[aLen][bLen];
        }
