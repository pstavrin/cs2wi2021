...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        ...
        IDictionary<IterableString, Integer> val = map.get(ngram);
        IPriorityQueue.PQElement<String>[] outSuggestions = new IPriorityQueue.PQElement[val.size()];
        int index = 0;
        for (IterableString key : val) {
            IPriorityQueue.PQElement<String> now = new IPriorityQueue.PQElement<>(key.toString(), val.get(key));
            outSuggestions[index] = now;
            index++;
        }
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        ...
        IPriorityQueue.PQElement<String>[] elem = getCountsAfter(ngram);
        TopKSort.sort(elem, k);
        String[] outSuggestions = new String[Math.min(k, elem.length)];
        for (int i = 0; i < outSuggestions.length ; i++) {
            outSuggestions[i] = elem[i].data;
        }
        return outSuggestions;
